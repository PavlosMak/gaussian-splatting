...
This repository is a fork of the original authors' implementation associated with the paper "3D Gaussian Splatting for Real-Time Radiance Field Rendering", which can be found [here](https://repo-sam.inria.fr/fungraph/3d-gaussian-splatting/). The idea for this fork is to be used to mainly to investigate the original technique. For datasets, pre-built binaries and other
information please refer to the [original repository](https://github.com/graphdeco-inria/gaussian-splatting).
...
```shell
# SSH
git clone git@github.com:PavlosMak/gaussian-splatting.git --recursive
```
or
```shell
# HTTPS
git clone https://github.com/PavlosMak/gaussian-splatting.git --recursive
```
...
> The script utility_scripts/run_viewer.sh can be used to launch the viewer. It simply executes the above command but can be useful to add as running configuration in your IDE.
...
> The shell script utility_scripts/video_to_frames.sh can be used to extract training data from a video. It requires ffmpeg to be installed.
